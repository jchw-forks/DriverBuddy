...
def check_for_fake_driver_entry(driver_entry_address):
    ...
    set_name(real_driver_entry_address, "Real_Driver_Entry", SN_CHECK)
...
def locate_ddc(driver_entry_address):
    ...
    for i in driver_entry_func[1:]:
        if ddc_offset in print_operand(i, 0)[4:] and print_insn_mnem(prev_instruction) == "lea":
            ...
            set_name(real_ddc, "DispatchDeviceControl", SN_CHECK)
        ...
        if didc_offset in print_operand(i, 0)[4:] and print_insn_mnem(prev_instruction) == "lea":
            ...
            set_name(real_didc, "DispatchInternalDeviceControl", SN_CHECK)
    ...
    for f in Functions():
        ...
        for i in instructions:
            ...
            if iocode in generate_disasm_line(i, 0):
                ddc_list.append(f)
    real_ddc = {}
...
def define_ddc(ddc_address):
    ...
    for i in instructions:
        disasm = generate_disasm_line(i, 0)
        ...
        if "rdx" in disasm and rdx_flag != 1 or irp_reg in disasm and irp_reg_flag !=1:
            ...
            # rdx got clobbered
                ...
                print("[+] rdx got clobbered %s" % generate_disasm_line(i, 0))
                rdx_flag = 1
            # irp_reg got clobbered
            elif print_insn_mnem(i) == "mov" and print_operand(i, 0) == irp_reg:
                irp_reg_flag = 1
            else:
                "[-] Something weird happened %s" % generate_disasm_line(i, 0)
            ...
            print("[+] io_stack_reg= %s in %s" % (io_stack_reg, generate_disasm_line(i, 0)))
            ...
            continue
            #print "[+] nothing interesting in %08x\nInstruction: %s" % (i, generate_disasm_line(i, 0))
